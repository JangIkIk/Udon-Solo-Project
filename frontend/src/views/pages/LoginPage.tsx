...
function LoginPage() {
    ...
    axios
      ...
      .then((res) => {
        ...
        if (res.data.userImage) {
          dispatch(userImage(res.data.userImage));
        }
  ...
  return (
    <Layout>
      {isModal === undefined ? null : (
        <Modal check={setIsModal} children="아이디와 비밀번호를 확인해주세요" />
      )}
      <div className="loginpage-info">
        <h1>로그인</h1>
        <input
          onChange={(text) =>
            setLoginData((prev) => ({ ...prev, loginId: text.target.value }))
          }
        ...
        <input
          onChange={(text) =>
            setLoginData((prev) => ({
              ...prev,
              loginPassword: text.target.value,
            }))
          }
        ...
        <label>
          <input
            onClick={() =>
              setLoginData((prev) => ({ ...prev, keepCheck: !prev.keepCheck }))
            }
            type="checkBox"
          />
          로그인 유지하기
