...
function SignUpPage() {
  ...
  const handleSubmit = ( event: React.FormEvent<HTMLFormElement>) => {
    ...
    axios.post<boolean>('/api/users/signup',{
  ...
  const findId = ( event : string )=>{
    axios.get<boolean>(`/api/users/signup/${event}`)
  ...
  return (
    <Layout>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <button style={{ "backgroundColor":"gray"}} className="form-submit-button" type="button" onClick={()=> navigate("/login")}> 로그인</button>
        <button style={{ "backgroundColor":"gray"}} className="form-submit-button" type="button" onClick={()=> navigate("/")}> 홈으로가기</button>
      </form>
    </Layout>
  );
}
